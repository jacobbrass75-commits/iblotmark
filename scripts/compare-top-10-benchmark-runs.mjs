...
const manifest = JSON.parse(await readFile(
  path.resolve(scriptDir, "../benchmarks/ai-visibility-top10-v1.json"),
  "utf8",
));
...
const compatibility = assessComparability(current, previous);
const currentStandardCheck = assessRunAgainstManifest(current, manifest);
const previousStandardCheck = assessRunAgainstManifest(previous, manifest);
if (!compatibility.comparable) {
  throw new Error(`Runs are not comparable: ${compatibility.blockers.join("; ")}`);
}
if (!currentStandardCheck.canonical || !previousStandardCheck.canonical) {
  const reasons = [
    ...currentStandardCheck.reasons.map((reason) => `current: ${reason}`),
    ...previousStandardCheck.reasons.map((reason) => `previous: ${reason}`),
  ];
  throw new Error(`Runs do not match ${manifest.id}: ${reasons.join("; ")}`);
}
...
const providerRows = (current.providerSummaries || []).map((row) => {
  ...
  if (!baseline) throw new Error(`Previous run is missing provider summary: ${row.provider}`);
  return {
...
const previousQueries = new Map((previous.querySummaries || []).map((row) => [
  normalizeBenchmarkText(row.query).toLowerCase(),
  row,
]));
const queryRows = (current.querySummaries || []).map((row) => {
  const baseline = previousQueries.get(normalizeBenchmarkText(row.query).toLowerCase());
  if (!baseline) throw new Error(`Previous run is missing query: ${row.query}`);
...
Protocol: \`${manifest.id}\` (comparison grade: **${compatibility.grade}**)
...
${compatibility.cautions.length > 0 ? `Historical protocol note: ${compatibility.cautions.join(" ")} The observable query, provider, prompt, model, and completion matrices still match exactly, so these deltas are provisional rather than strict.` : "The stored protocol, query, provider, prompt, model, scorer, and completion matrices match exactly."}
...
await Promise.all([
  writeFile(path.join(outputDir, "comparison.json"), `${JSON.stringify({
    currentRunId,
    previousRunId,
    benchmarkStandard: manifest.id,
    compatibility,
    providerRows,
    queryRows,
  }, null, 2)}\n`, "utf8"),
...
console.log(JSON.stringify({
  ...
  comparisonGrade: compatibility.grade,
  providerRows,
}, null, 2));
