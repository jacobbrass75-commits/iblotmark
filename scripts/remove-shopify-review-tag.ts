...
const OUT_PATH = "content-output/faq-format-backups-2026-07-06/shopify-review-tag-removal.json";
const REVIEW_TAG_PATTERN = /^ibol?t-mark-review$/i;
...
for (const blogId of BLOG_IDS) {
  ...
  for (const article of articles) {
    ...
    const nextTags = tags.filter((tag) => !REVIEW_TAG_PATTERN.test(tag));
    ...
    results.push({
      ...
      removedTags: tags.filter((tag) => REVIEW_TAG_PATTERN.test(tag)),
      previousTags: tags,
    ...
    console.log(`Removed review tag from ${article.title}`);
  }
}
...
console.log(`Removed review tag from ${results.length} article(s).`);
